...
- **Tooth shape: a dart** (settled 2026-09-11 after a flat dart with fillets, git 49ceeaa, "not bad, but ugly", and a short canoe-fairing pod, git 506b449; Armen's reference was the flap-track pods under an airliner's wing). The tooth is a body of revolution about an axis along Z: a long elliptical nose (`tooth_nose = 8`) reaching `tooth_prong = 11` below the rim, toward the shroud's mid-height, cylinder `tooth_r = 3` through the rim, cone tail `tooth_tail = 7` fading up the bar's inside face (`tooth_pod_profile_2d`, revolved and D-clipped by `tooth_d_sweep`). The prongs grip wherever the pod is fatter than the wall's half-band, about 10 of the 11 mm. The axis sits on the wall's mid-radius at `tooth_ang = 52.5`°, the middle of the gap between the deco boxes at 45° and 60°: the -Y tooth's outer prong reaches down among the boxes, and at 54° / r 3.25 it grazed one. Because the pod is round, the wall splits it into two D prongs no matter how obliquely the bar crosses the ring. The pod is clipped flat at the bar's outer face (2.1 mm from the axis) so the vane still prints flat; the bulge is on the inside face with the hinge barrels. The -Y tail ends just under the slat hole; the slat hole cut runs long to pass any pod. Renders: `renders/fork_tooth.png` (inside), `renders/fork_tooth_outside.png`, `renders/vane_root_side.png`.
- **Rounded edges** (2026-09-11, "they look too blocky"): the bar and the plate are built with `rounded_plate()` from `../lib/shape.scad` (outline shrunk by r, extruded, minkowski'd with a sphere), `vane_edge_r = 0.7` on every edge of both faces; convex outline corners come out rounded too. On the bed the fillet's first layer is inset 0.2 mm, well within what the H2C bridges. The hinge webs, knuckle ends, tie bar and slats are still sharp-edged. Render: `renders/vane_edges.png`.
