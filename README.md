...
- **Fork teeth** (2026-09-11, replacing the drop-in pegs, which sat loose and rocked): each tooth is a dart in the bar's plane (`tooth_dart_2d`, `tooth_profile_2d`, `tooth_fork`): 11 wide where it leaves the bar with 2.5 mm concave fillets, straight sides converging to a round nose (`tooth_tip_r = 3.5`, tip 7 wide), and the inside face sloping from 2 mm at the key's bottom to `tooth_tip_t = 1` at the nose. Armen's brief: "swoop into the duct, slightly conical, like the lights on an art-deco wing". The slot between the prongs is the duct wall's own annulus offset `tooth_clr`. The key in the middle drops `tooth_key = 3` into a notch in the rim; the two prongs run `tooth_prong = 7` below the rim top on both faces of the wall and grip it. The slot follows the curve, so the fit is tight even though the bar crosses the ring at ~50°. Slot mouth chamfered 0.6. The click nub sits on the key's inside face at the wall's mid-radius. Renders: `renders/fork_tooth.png`, `renders/fork_tooth_sec.png`, `renders/vane_root_side.png`. The outer prongs are visible on the shroud's outside between deco boxes, clear of them by ~2°.
- How the fork locates: the notch is wider along the bar than the 4.4 mm band the wall occupies there, so it is really a 2.3 mm slot straight across the wall. Its two x faces stop the vane sliding along the rim (and the assembly spinning about the duct axis); the prongs stop radial play; the key's bottom on the notch floor is the Z stop. The dart's taper is cosmetic and does not touch the fit.
