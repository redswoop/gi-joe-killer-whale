...
| `viewer.json` | Part list for the browser viewer (`../../viewer`, `bun run dev`, open http://127.0.0.1:5180/whale/shroud/). |
...
- **Fan** (`fan()`, prints flat, hub bottom on the bed, no supports): hub `fan_hub_d` 10 x `fan_hub_h` 7 with a blind
  ...
  spline as their edge ... built between 2 smooth splines'): each edge is a cubic Bezier in a unit frame (1 = `fan_r`;
  `blade_te_c`, `blade_le_c` inner control points), both arriving at `blade_tip` along `blade_tip_dir` from opposite
  sides so the tip is a smooth round of size `blade_tip_k` (0.06: sharp and swept; 0.18 was a fat paddle). **Root**
  (Armen: 'they should end inside of the hub, with the edge fully in contact with the hub ... extrude from a projected
  profile on the hub'): both edges start ON the hub circle, +-`blade_root_a` about the blade's axis so the root chord
  (`blade_root_c` 0.15) is a chord of the hub, and that section is extruded straight inward `blade_bury` 1.5 before the
  Bezier stations begin; the root cap stays 2.5 from the axis, clear of the socket. **Section** (`blade_profile()`):
  flat bottom on the bed, a full round nose of diameter `blade_t` 1.0 at the leading edge, the top on a cosine from
  there up to the trailing edge's thickness, blade_t + chord x tan `blade_pitch` 12 deg capped at `blade_t_max` 2.2
...
- **Shaft** (`shaft()`, prints lying on its flat): D section, `shaft_d` 3.5 (the reference's; the strut's bore is 5)
  ...
  slot, `tab_w` 2.8 x `tab_t` 1.2 x `tab_len` 3.0 (guessed from the photos; **the first print fit the gear**, 2026-09-12; the mesh's
  1.6 x 1.2 x 0.6 pin was a guess), end corners rounded `tab_end_r`, and the shoulders where the round shank becomes the blade are
...
- **Teeth** (2026-09-11). The two ends of each bar differ:
  - **+Y (toy's bottom): the original Sketch 06 peg**, `peg()`: the bar's thickness, 5.27 wide (y 31.9..37.2), 2.2 mm below the rim (z 19.065), sharp-edged, no nub. It drops through the rim slot into a small slot in the Whale's base, so from the rim down it keeps the sketch's size exactly ("the slot is quite small, 1-2 mm depth max"). The rim slot is the peg + `peg_clr`. **Root flared (2026-09-12)**: the first printed vane snapped off at this root (sharp inside corner on a 2 x 5.27 section). `peg_2d()` now flares the peg into the bar's bottom edge with `peg_fillet = 3` concave fillets on both sides, in the bar's plane, using 3 of the 4 mm between the bar's edge and the rim top (they stop ~1 mm above the rim, so the notch and the base slot see the plain peg). Extruded at the full 2 mm, so it stands 0.06 proud of the tapered bar over that strip. **Canoe (2026-09-12)**: `peg_pod()` wraps the root in a body of revolution on the bar's mid-plane, `peg_pod_r = 2.75` (just over the peg's half-width), full round to `tooth_over` past the bar's edge, cone tail `peg_pod_tail = 5` fading into the bar (added after the panel grooves are cut, so it rides smoothly over the ribbed panel). Below `peg_pod_clr = 0.5` above the rim, only the part inside the bore survives: a half-canoe hugging the bore wall (r < 44 - `tooth_clr`) with an ogive nose `peg_pod_nose = 6`, tip `peg_pod_prong = 8` below the rim, so the root also grips the inside of the wall like the dart's inner prong. Outside the bore nothing reaches below the rim: the rim notch and the base's slot see only the plain peg. Renders: `renders/peg.png`, `renders/peg_iso.png`, `renders/peg_bore.png`.
...
- Vane print 2026-09-11 (Armen, 45°, forks toward the bed): hinge perfect; **the fork darts broke off during support removal** (1.1 mm prongs). Second print lying on its side, hinges down, support PLA with 0 interface distance: **the root then snapped off the duct at the +Y peg** (2026-09-12) -> peg root flared with 3 mm fillets, unprinted. Armen wants the bottom mount "more beefy" and keeps the canoes at the top; if the fillets are not enough, the 4 mm of clearance above the rim allows a taller shoulder (`peg_fillet` up to ~3.5) before the bar itself would have to move.
...
- Fan and shaft (2026-09-12): unprinted. Blade fits the gear (printed 2026-09-12). Open: `blade_hand` (gearbox direction), `shaft_extra` (how much longer), `shaft_clr`
