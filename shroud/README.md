...
| `shroud_makerworld.scad` | **MakerWorld customizer build**: one self-contained file (lib inlined, `part` dropdown, curated parameters on top, the rest under `[Hidden]`), because the Parametric Model Maker takes a single file and runs OpenSCAD 2021.01. GENERATED by `build_makerworld.py` from `shroud.scad` + `print_layout.scad` + `../lib/`: re-run it after any change, then re-upload the file on MakerWorld. Verified 2026-09-13: every part renders with the same volume, area and bbox as `print_layout.scad` (zero voxel diff). |
